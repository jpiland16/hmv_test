...
const inputData = `1.0 0.0 0.0 0.0
...
export default function GeneratedOrientation(props) {
    ...
    if (props.timeSliderValue !== props.lastIndex.current) {
        ...
        //rotation arm outward
        let upwardAxis = new THREE.Vector3(0,1,0)
        upwardAxis.applyQuaternion(q)
        let outRotation = new THREE.Quaternion();
        outRotation.setFromAxisAngle(upwardAxis, Math.PI/2);
        q.multiply(outRotation)

        //apply rotation that sets x to the desired direction
        let q_data = getQuaternionFromLine(props.timeSliderValue)

        console.log("Quaternion from data: ");
        console.log(q_data);
        let xVector = new THREE.Vector3(1, 0, 0);
        xVector.applyQuaternion(q_data);
        console.log("Modified x vector: ");
        console.log(xVector);
        let yVector = new THREE.Vector3(0, 1, 0);
        yVector.applyQuaternion(q_data);
        console.log("Modified y vector: ");
        console.log(yVector);

        q.multiply(q_data);
        console.log("Full applied quaternion: ");
        console.log(q);
        q = q_data;
